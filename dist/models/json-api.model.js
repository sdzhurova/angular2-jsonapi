...
var JsonApiModel = (function () {
        ...
        if (hasMany) {
            for (var _i = 0, hasMany_1 = hasMany; _i < hasMany_1.length; _i++) {
                ...
                if (relationship && relationship.data && relationship.data.length > 0) {
                    var relationshipModel = this.getHasManyRelationship(relationship.data, included, level);
    ...
    JsonApiModel.prototype.getHasManyRelationship = function (data, included, level) {
        var _this = this;
        var relationshipList = [];
        data.forEach(function (item, index) {
            var typeName = data[index].type;
            var modelType = Reflect.getMetadata('JsonApiDatastoreConfig', _this._datastore.constructor).models[typeName];
            if (!modelType) {
